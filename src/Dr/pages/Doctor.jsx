...
// //   {doctorProfile?.room ? `Room ${doctorProfile.room} - Floor ${doctorProfile.floor}` : 'Room 205 - Floor 2'}
// // </p>
// //           </div>
...
//   // API Data
//   const [doctorProfile, setDoctorProfile] = useState(null);
//   const storedUser = localStorage.getItem('patient') ? JSON.parse(localStorage.getItem('patient')) : null;
//   const [patientQueue, setPatientQueue] = useState([]);
//   const [currentPatient, setCurrentPatient] = useState(null);
//   const [diagnosis, setDiagnosis] = useState('');
//   const [notes, setNotes] = useState('');
...
//           <div>
//             <h1 className="text-2xl font-bold text-[#2C5F8D]">
//               {doctorProfile?.name || storedUser?.name}
//             </h1>
//             <p className="text-sm text-[#5B9BD5]">
//             {doctorProfile?.specialty || storedUser?.specialty || ''}
//             </p>
//     <p className="text-xs text-slate-500">
//   {doctorProfile?.room_number || storedUser?.room_number ? `Room ${doctorProfile?.room_number || storedUser?.room_number} - Floor ${doctorProfile?.floor_number || storedUser?.floor_number}` : ''}
// </p>
//           </div>
//           <div className="flex gap-3 items-center max-sm:justify-between">
//             <button className={`px-5 py-2 rounded-lg font-medium text-sm ${
...
//               <div className='bg-white p-3'>
//                 <h2>Ai Analysis</h2>
//                 <div className="space-y-1.5 text-sm leading-relaxed mt-2">
//                   {(xrayFiles[currentXrayIndex]?.ai_analysis || 'No AI analysis available')
//                     .split('\n').map((line, i) => {
//                       const t = line.trim();
...
//                       if (t.startsWith('- ') || t.startsWith('• '))
//                         return <div key={i} className="flex gap-2 ml-2"><span className="text-blue-400 mt-0.5">•</span><span className="text-gray-700">{t.replace(/^[-•]\s*/, '').replace(/\*\*/g, '')}</span></div>;
//                       if (/^\d+[\.]\s/.test(t))
//                         return <div key={i} className="flex gap-2 ml-2"><span className="text-blue-800 font-bold min-w-[20px]">{t.match(/^\d+/)[0]}.</span><span className="text-gray-700">{t.replace(/^\d+[\.]\s*/, '').replace(/\*\*/g, '')}</span></div>;
//                       return <p key={i} className="text-gray-700">{t.replace(/\*\*/g, '')}</p>;
//                     })}
//                 </div>
//               </div>
//             </div>
...
//   // ✅ helper لتحويل الـ response لـ array
//   const toArray = (data) =>
...
//   useEffect(() => {
//     loadInitialData();
//     startQueuePolling(); // ✅ ابدأ الـ polling

//     return () => stopQueuePolling(); // ✅ وقف لما الصفحة تتقفل
//   }, []);
...
//         stopQueuePolling(); // ✅ وقف الـ polling لما يكون في consultation
...
//   const toggleStatus = async () => {
//     const newStatus = !isAvailable;
//     setIsAvailable(newStatus); // غيّر الـ UI فوراً
//     try {
//       setLoading(true);
//       await doctorAPI.updateStatus({ available: newStatus });
//     } catch (err) {
//       setIsAvailable(!newStatus); // لو فشل ارجع للقديم
//       setError('Failed to update status: ' + err.message);
//     } finally {
//       setLoading(false);
//     }
...
//       if (response.patient) {
//         // الـ API بيرجع patient_id مش id
//         const patientId = response.patient.patient_id || response.patient.id;
//         const patientDetails = await doctorAPI.getPatient(patientId);
//         // دمج بيانات الـ queue مع بيانات الـ patient
//         const mergedPatient = {
//           ...patientDetails,
//           queue_id: response.patient.queue_id,
//           name: patientDetails?.name || patientDetails?.patient_name || response.patient.patient_name,
//           severity: response.patient.severity_level,
//         };
//         setCurrentPatient(mergedPatient);
//         setIsConsulting(true);
//         isConsultingRef.current = true;
//         stopQueuePolling(); // ✅ وقف الـ polling لما يبدأ consultation
...
//       const updatedQueue = await doctorAPI.getQueue();
//       setPatientQueue(toArray(updatedQueue));
//       startQueuePolling(); // ✅ رجّع الـ polling بعد ما يخلص consultation
...
//       const updatedQueue = await doctorAPI.getQueue();
//       setPatientQueue(toArray(updatedQueue));
//       startQueuePolling(); // ✅ رجّع الـ polling
...
//                           if (Array.isArray(diseases)) {
//                             return diseases.length > 0 
//                               ? diseases.map((disease, i) => <li key={`disease-${i}`} className="text-slate-700">• {disease}</li>)
//                               : <li className="text-slate-500">None reported</li>;
//                           }
//                           if (typeof diseases === 'string' && diseases.trim()) {
//                             return <li className="text-slate-700">• {diseases}</li>;
...
//                           if (Array.isArray(allergies)) {
//                             return allergies.length > 0
//                               ? allergies.map((allergy, i) => <li key={`allergy-${i}`} className="text-slate-700">• {allergy}</li>)
//                               : <li className="text-slate-500">None reported</li>;
//                           }
//                           if (typeof allergies === 'string' && allergies.trim()) {
//                             return <li className="text-slate-700">• {allergies}</li>;
...
//                           if (Array.isArray(medications)) {
//                             return medications.length > 0
//                               ? medications.map((med, i) => <li key={`med-${i}`} className="text-slate-700">• {med}</li>)
//                               : <li className="text-slate-500">None reported</li>;
//                           }
//                           if (typeof medications === 'string' && medications.trim()) {
//                             return <li className="text-slate-700">• {medications}</li>;
...
//   // Normalize patient - handles nested `patient` object from API
...
//   // ✅ جيب الصورة كـ blob عشان تتجاوز الـ ngrok browser warning
...
//         }
//       });
//       if (!res.ok) return url; // fallback للـ URL الأصلي
...
//     try {
//       const queue = await doctorAPI.getQueue();
//       // فلتر أي مريض status بتاعه called أو in_progress من الـ queue
...
//       // ✅ حوّل كل URL لـ blob عشان تتجاوز ngrok warning
...
//     // ✅ جيب الـ treatment history من الـ API
//     try {
//       const BASE_URL = 'https://subrhombical-akilah-interproglottidal.ngrok-free.dev';
//       const token = localStorage.getItem('token');
...
//     setError(null);
//     try {
//       // ✅ FIX: جيب الـ profile من الـ API وحدّث isAvailable منه
//       let doctorData = null;
//       // ✅ دايماً جيب الـ profile من الـ API عشان يبقى محدّث
//       try {
//         const profile = await doctorAPI.getProfile();
...
//       // ✅ FIX: حدّث isAvailable من بيانات الدكتور الفعلية
//       if (doctorData) {
...
//   // ✅ FIX: toggleStatus مع error handling صح
//   const toggleStatus = async () => {
//     const newStatus = !isAvailable;
//     setIsAvailable(newStatus); // غيّر الـ UI فوراً
//     try {
...
//     } catch (err) {
//       setIsAvailable(!newStatus); // لو فشل ارجع للقديم
//       // ✅ FIX: استخرج الـ message صح من أي نوع error
...
//   // ✅ FIX: callNextPatient مع handling أفضل
//   const callNextPatient = async () => {
//     if (!isAvailable || patientQueue.length === 0) return;
...
//           // ✅ FIX: لو getPatient فشل، استخدم البيانات الموجودة في response
//           patientDetails = response.patient;
//         }

//         // ✅ FIX: دمج صح لأي structure ترجع من الـ API
//         const mergedPatient = {
//           ...response.patient,
//           ...patientDetails,
...
//         // ✅ sync مع الـ server - بس شيل المريض المستدعي من النتيجة
//         try {
//           const updatedQueue = await doctorAPI.getQueue();
...
//       // ✅ امسح بيانات المريض الحالي بس، مش الـ selectedDoctor
//       localStorage.removeItem('currentPatient');
//       localStorage.removeItem('queueData');
...
//         // ✅ 3: ادمج الـ ai_analysis في الـ result
//         let aiText = null;
//         if (aiResult) {
...
//           ...result,
//           file_url: getFullUrl(result.file_url || result.url),
//           ai_analysis: aiText,
//           title: result.title || aiResult?.suggested_title,
//           created_at: result.created_at || aiResult?.analyzed_at,
//         };

//         setUploadedXray(enrichedResult.file_url || URL.createObjectURL(file));
...
//         setUploadedLab(enrichedResult.file_url || URL.createObjectURL(file));
...
//           </div>
//           <div className="flex gap-3 items-center max-sm:justify-between">
//             {/* ✅ FIX: Status badge يعرض الحالة الصح */}
...
//           {/* Main Patient Panel */}
//           <div className="p-4 sm:p-6 lg:p-8 space-y-6">
//             {/* ✅ FIX: الشرط بيشيك على isConsulting و currentPatient مع بعض */}
//             {!isConsulting || !currentPatient ? (
//               <div className="flex flex-col items-center justify-center h-full text-slate-400 space-y-4">
//                 <span className="text-8xl">🏥</span>
//                 <h2 className="text-2xl font-bold text-slate-500">No Active Patient</h2>
...
//                 <h2>Ai Analysis</h2>
//                 <div className="mt-2 space-y-3">
//                   {(xrayFiles[currentXrayIndex]?.ai_analysis || 'No AI analysis available').split(/\d+\.\s+/).filter(Boolean).map((point, i) => {
//                     const importantSplit = point.split(/⚠️\s*IMPORTANT:|IMPORTANT:/);
//                     const mainText = importantSplit[0];
//                     const important = importantSplit[1];
//                     const titleMatch = mainText.match(/^\*\*(.+?)\*\*[:\s]*(.*)/s);
...
//                       <div key={i} className="text-sm text-slate-700">
//                         <span className="font-bold text-slate-900">{i + 1}. {titleMatch ? titleMatch[1] : ''}</span>
//                         {titleMatch ? <span className="text-slate-600"> {titleMatch[2].trim()}</span> : <span> {mainText.trim()}</span>}
//                         {important && <p className="mt-1 text-orange-600 font-semibold">⚠️ IMPORTANT: {important.trim()}</p>}
//                       </div>
//                     );
//                   })}
//                 </div>
...
//                 <h2 className='my-3'>Ai Analysis</h2>
//                 <div className="space-y-3">
//                   {(labTestFiles[currentLabIndex]?.ai_analysis || 'No AI analysis available').split(/\d+\.\s+/).filter(Boolean).map((point, i) => {
//                     const importantSplit = point.split(/⚠️\s*IMPORTANT:|IMPORTANT:/);
//                     const mainText = importantSplit[0];
//                     const important = importantSplit[1];
//                     const titleMatch = mainText.match(/^\*\*(.+?)\*\*[:\s]*(.*)/s);
...
//         // لو الـ API رجع بيانات ناقصة (مفيش floor أو room)، كمّلها من queueData
...
//         // ✅ استخدم الـ local URL أولاً للعرض الفوري بدون مشاكل ngrok
