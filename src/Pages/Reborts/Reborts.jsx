...
//   const [mode, setMode] = useState('now'); // 'now' or 'scheduled'
...
//   // Load specialties + existing appointments
//   useEffect(() => {
...
//   // Load slots when doctor + date changes (scheduled mode only)
//   useEffect(() => {
...
//     setSubmitting(true);
//     try {
//       const result = await api.joinQueue({ patient_id: pid, doctor_id: selDoc.id, severity_level: 5 });
...
//     } catch (e) {
//       const msg = e?.message || '';
//       if (msg.includes('already in')) { toast.error('أنت في الطابور بالفعل!'); setTimeout(() => navigate('/wating'), 1000); }
//       else toast.error(msg || 'فشل الحجز');
...
//     } catch (e) {
//       // ✅ التعديل - بنقرأ الـ response من الـ Backend صح
...
//         // ✅ حفظ بيانات الدكتور على الأقل عشان صفحة الانتظار متفشلش
